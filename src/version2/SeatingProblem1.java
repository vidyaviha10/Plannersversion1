...
public class SeatingProblem1 {
    ...
    public ArrayList<Integer> showOptimalList() {
        ...
        for (int y = 0; y < this.number_of_items; y++) {
            if (optimal_list.substring(y, y + 1).equals("1")) {
                System.out.print((y + 1) + " "+" name: "+this.Guests.get(y).name);
                finalls.add(y + 1);
            }
        }
    ...
    /*    if (rand_mutation <= this.prob_mutation)*/ {
    	...
    	String falt = "";
    	 for(int k = 0;k<this.breed_population.size()-2;k++)
    	 {
    		 try{
    			 System.out.println();
    			 falt = this.breed_population.get(k);
    			 
    			/* ensureSize(this.breed_population.get(k));
    			 this.breed_population.set(this.breed_population.size() - 1, new_mut_gene);*/
    			 
    			 
    	int id = getHighestWeightInGene(this.breed_population.get(k));
    	System.out.println("this.breed_population.get(k): "+this.breed_population.get(k)+" highest id: "+id);
String gene = mutatewithHighestGene(id,this.breed_population.get( k));

this.breed_population.set(k, gene);
    		 }
    		 catch(Exception e)
    		 {
    			 e.printStackTrace();
    			 System.out.println("##################### gene after my mutation "+falt);
    		 }
    	 }
    ...
    public void crossoverGenes(final int gene_1, final int gene_2) {
        ...
        if (rand_crossover <= this.prob_crossover) {
            ...
            this.breed_population.add(ensureSize(this.population.get(gene_1)));
            this.breed_population.add(ensureSize(this.population.get(gene_2)));
        }
    ...
    public double evalGene(final String gene) {
        ...
        for (int j = 0; j < this.number_of_items; j++) {
            ...
            if (c == '1') {
                total_weight = total_weight + this.Guests.get(j).weight;
                //System.out.println("this.weight_of_items.get(j): "+this.Guests.get(j).weight);
             
            }
        }
        
        total_weight = evaluateConstraints(gene,total_weight);
...
System.out.println("!!!!!!: "+ this.Guests.get(i).weight);
            map.put(i, this.Guests.get(i).weight);
    ...
    public String makeGene() {
        ...
        for(int l = 0;l<this.table_size;l++)
        {
        	
        	
        	 Random r = new Random();
         	int number = r.nextInt((this.number_of_items-1 - 0) + 1) + 0;
         	tobe1.add(number);
        }
    ...
    public static <K, V extends Comparable<? super V>> List<Entry<K, V>> entriesSortedByValues(final Map<K, V> map) {
    ...
    int getHighestWeightInGene(String gene)
    {
    	//Map<String,Integer> largest = new HashMap();
    	...
    	for(int i =0;i<gene.length();i++)
    	{
    		if(gene.charAt(i) == '1')
    		{
    			if(Guests.get(i).weight > largestweight)
    			{
    				id =i;
    				largestweight = Guests.get(i).weight;
    			}
    		}
    	}
    ...
    String mutatewithHighestGene(int id,String gene)
    {
    	String toSit = Guests.get(id).toSit;
    	String nottoSit = Guests.get(id).nottoSit;
    	Integer ids = id;
    	StringBuilder genes = new StringBuilder(this.number_of_items);
    	int counter =0;
    	System.out.println("gene: "+gene+" length: "+gene.length());
    	if(toSit == null || toSit.trim().equals(""))
    	{
    		return gene;
    	}
    	else
    	{
    		char c = '0';
    		for(Integer i =0;i<gene.length();i++)
        	{
        		...
        		else if(doesContainsitwith(Guests.get(i).getId(),toSit))
        		{
        			c = '1';
        			 counter++;
        	            if (counter == this.table_size) {
    		...
    		System.out.println("initial gene mutation: "+ genes.toString());
    	for(Integer i = counter;i<this.table_size;i++)
    	{
    		Random r = new Random();
         	int number =  r.nextInt((this.number_of_items -1 - 0) + 1) + 0;
         	System.out.println("$%$: "+genes.length()+" number "+number);
         	if(genes.charAt(number) != '1' && !doesContainnotsitwith(number+1,nottoSit) && loworderContraints(genes,number))
         	{
         		genes.setCharAt(number, '1');
   			
         	}
    	}
    	System.out.println("genes.toString(): "+genes.toString()+" gene: "+gene);
    		return genes.toString();
    	}
    
    }
    
    double evaluateConstraints(String gene, double weight)
    {
    	System.out.println("evaluateConstraints "+gene);
    	outer:for(int i =0;i<gene.length();i++)
    	{
    		if(gene.charAt(i) =='1')
    		{
    		String toSit = Guests.get(i).toSit;
        	String nottoSit = Guests.get(i).nottoSit;
        	
        	for(Integer k =0;k<gene.length();k++)
        	{
        		if(gene.charAt(k) =='1')
        		{
        			if(doesContainsitwith(Guests.get(k).getId(),toSit))
        			{
        				weight = weight+Guests.get(getHighestWeightInGene(gene)).getWeight();
        			}
        			else if(doesContainnotsitwith(Guests.get(k).getId(),nottoSit))
        			{
        				weight = 0.0;
        				return weight;
        			}
        		}
        		else if(gene.charAt(k) =='0')
        		{
        			if(doesContainsitwith(Guests.get(k).getId(),toSit))
        			{
        				weight = weight-Guests.get(getHighestWeightInGene(gene)).getWeight();
    ...
    boolean doesContainsitwith(Integer i, String value)
    {
    	System.out.println("doesContain: "+i+" value "+value);
    	...
    	else if(value.equals(i.toString()))
    	{
    		System.out.println("returning true");
    		return true;
    	}
    ...
    boolean doesContainnotsitwith(Integer i, String value)
    {
    	System.out.println("doesContain: "+i+" value "+value);
    ...
    boolean loworderContraints(StringBuilder genes,Integer number)
    {
    	String gene = genes.toString();
    	/*String allone ="";
    	String nottoSit = Guests.get(number).getNottoSit();
    	String toSit = Guests.get(number).getToSit();
    	for(int i =0;i<gene.length();i++)
    	{
    		if(gene.charAt(i)=='1')
    		{
    			if(doesContain(i+1,nottoSit) );
    			{
    				return false;
    			}
    			else if(doesContain(i+1,toSit))
    			{
    ...
    boolean flag = true;
    	{
    		...
    		{
    		String toSit = Guests.get(number).toSit;
        	String nottoSit = Guests.get(number).nottoSit;
